...
QMapboxGLPrivate::QMapboxGLPrivate(QMapboxGL *q, const QMapboxGLSettings &settings, const QSize &size_, qreal pixelRatio_)
    : QObject(q)
    , size(size_)
    ...
    , threadPool(mbgl::sharedThreadPool())
{
    ...
    // Setup MapObserver
    mapObserver = std::make_unique<QMapboxGLMapObserverPrivate>(this);
...
{
    if (!m_backend) {
        m_backend = std::make_unique<QMapboxGLRendererBackendPrivate>(
                pixelRatio, *fileSourceObj, *threadPool, mode);
    }
